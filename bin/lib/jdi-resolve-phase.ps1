...
#
# Usage (from a command MD file):
#   $r = & "$JDI_LIB\jdi-resolve-phase.ps1" -Id $phaseInput
#   if ($LASTEXITCODE -ne 0) { Write-Error $r; exit $LASTEXITCODE }
#   $r | Invoke-Expression  # imports $JDI_PHASE_* variables
#
# Or capture object:
#   $r = & "$JDI_LIB\jdi-resolve-phase.ps1" -Id $phaseInput -AsObject
#
# Emits Bash-style KEY='value' lines by default for parity with the .sh
# version. Pass -AsObject to receive a PSCustomObject instead.
#
# Exit codes:
#   0  resolved
#   1  invalid input
