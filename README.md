...
├── .gitattributes                 # EOL normalization + merge=union on append-only .jdi files
...
- `DECISIONS.md` is **append-only**. Locked decisions never reverse — and the reviewer verifies it: Gate 6 BLOCKS a diff that contradicts a locked decision. v1 uses `D-1`, `D-2`, …; v2 uses deterministic `D-YYYY-MM-DD-slug-seq` (collision-free across developer branches). Genesis decisions keep `D-1` (code design) / `D-2` (adoption boundary)
...
- **ROADMAP.md auto-merges too (0.11.0+).** `/jdi-issue` made phase appends a per-card operation, so the ROADMAP now carries `merge=union` and stores NO `total_phases` counter (derived from heading count — stored aggregates conflict on every parallel add and merge stale). Parallel `/jdi-issue`/`add-phase` appends merge clean. Known trade-off, rare and recoverable: a `remove-phase` racing a merge can resurrect the removed block — it stays audited (`D-{date}-{slug}-rm` in DECISIONS.md) and re-running the remove fixes it. After merging parallel appends, display numbers may duplicate (`### Phase 2` twice) — cosmetic only, slugs are the identity; renumber whenever convenient. Inserting mid-roadmap (`--before`/`--after`) on two branches at the same spot may still conflict — that is deliberate planning, do it on an up-to-date branch.
