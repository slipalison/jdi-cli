...
type: skill
applies_to: |
  Carregada quando PROJECT.md tem `frontend.has_frontend: true`.
  Doer aplica como guia ANTES de escrever/editar files de UI.
  Reviewer aplica como checklist em gate 5 (security/perf rules) antes do gate 7 (UI live).
loaded_by:
  - jdi-doer-{slug} (condicional a frontend.has_frontend)
  - jdi-reviewer-{slug} (condicional a frontend.has_frontend)
runtime_overrides:
  antigravity:
    triggers:
      - "regras de frontend"
