...
| Per-project specialist (`.jdi/agents/jdi-doer-*`, `jdi-reviewer-*`) | same path, generated by `jdi-cli sync-specialists` (committed; `name/description/model/tools`) | same, `.agent.md` (`tools` mapped to Copilot verbs) | same, as a skill (`triggers`) | same (`mode: subagent` + `permission`) | same (`tools` allowlist + `reasoningLevel`) |
